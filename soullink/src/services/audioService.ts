...
export type AudioEmotionDetectionResult = {
  voiceEmotion: string;
  transcript: string;
  available: boolean;
};
...
class AudioService {
  ...
  async detectAudioEmotion(audioBlob: Blob): Promise<AudioEmotionDetectionResult> {
    try {
      ...
      console.log('[AudioService] Starting voice emotion detection:', audioBlob.size, 'bytes');

      const response = await fetch(`${FAY_API_URL}/api/detect_audio_emotion`, {
        method: 'POST',
        body: formData,
      });
      const result = (await response.json().catch(() => null)) as DetectAudioEmotionResponse | null;

      if (!response.ok) {
        throw new Error(result?.message || `Voice emotion detection failed: ${response.status}`);
      }

      const voiceEmotion = String(result?.voice_emotion || result?.message || '').trim();
      const transcript = String(result?.transcript || '').trim();
      const available = result?.available !== false && Boolean(voiceEmotion);

      console.log('[AudioService] Voice emotion detection result:', voiceEmotion || 'unavailable');
      ...
      console.error('[AudioService] Voice emotion detection failed:', error);
      throw error;
    }
  }
