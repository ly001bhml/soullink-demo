...
from ai_module import emotion_router
from ai_module import emotion_state
...
elif cfg.tts_module == 'voxcpm2':
    from tts.voxcpm2_tts import Speech
else:
...
class FeiFei:
        ...
        if self.__running:
            try:
                ...
                if index == 1: #语音、文字交互
                    ...
                    # emotion detection and lightweight care-state generation
                    try:
                        emotion_label, emotion_id = emotion_router.detect_text_emotion(interact.data["msg"])
                        input_source = "voice" if interact.interleaver in ("mic", "socket") else "text"
                        state = emotion_state.build_emotion_state(
                            interact.data["msg"],
                            input_source=input_source,
                            text_label=emotion_label,
                            text_raw_id=emotion_id,
                            voice_hint=interact.data.get("voice_emotion_hint"),
                        )
                        interact.data["emotion_state"] = state
                        interact.data["emotion"] = state.get("valence")
                        if emotion_id is not None:
                            interact.data["emotion_id"] = emotion_id

                        observation_line = emotion_state.format_emotion_observation(state)
                        existing_obs = interact.data.get("observation")
                        if observation_line:
                            if existing_obs:
                                interact.data["observation"] = f"{existing_obs}\n{observation_line}"
                            else:
                                interact.data["observation"] = observation_line

                        if wsa_server.get_instance().is_connected(username):
                            content = {'Topic': 'human', 'Data': {'Key': 'emotion', 'Value': state.get("valence")}, 'Username': username}
                            wsa_server.get_instance().add_cmd(content)

                        try:
                            content_db.new_instance().add_emotion_event(
                                content_id,
                                username,
                                uid,
                                model_id,
                                state.get("valence"),
                                emotion_source=state.get("source", input_source),
                                emotion_id=emotion_id,
                            )
                        except Exception as exc:
                            util.log(1, f"[Emotion] 记录情绪事件失败: {exc}")
                    except Exception as exc:
                        util.log(1, f"[Emotion] 情绪状态处理失败: {exc}")
                    ...
                    if answer is None or type != "qa":
                        ...
                        if wsa_server.get_instance().is_connected(username):
                            ...
                            wsa_server.get_instance().add_cmd(content)
                        text = nlp_cognitive_stream.question(interact.data["msg"], username, interact.data.get("observation", None), interact.data.get("pure_mode", False), interact.data.get("system_override"))
    ...
    def __get_mood_voice(self, username=None):
        ...
        参数:
            username: 用户名（可选）
        """
        voice_name = "zhifeng_emo"  # 默认知锋_多情感
        ...
        if voice_name == "zhifeng_emo":
            try:
                cfg.load_config()
                voice_name = cfg.config.get("attribute", {}).get("voice", "zhifeng_emo")
            except Exception:
                pass
    ...
    def say(self, interact, text, type = ""):
        try:
            ...
            if not is_first and not is_end and (text is None or text.strip() == ""):
                return None
            text = self.__normalize_childlike_tone_text(text)
            ...
            if result is not None or is_first or is_end:
                if is_end:
                    # 结束标记时短暂等待，避免 is_end 先于前一段 TTS 到达；0.25s 足够，1s 会导致两段间明显停顿
                    time.sleep(0.25)
    ...
    def __normalize_childlike_tone_text(self, text):
        """
        主链路文本占位函数（当前不做文案改写，保持模型原始输出）。
        """
    ...
    def __process_output_audio(self, file_url, interact, text):
        try:
            ...
            if file_url is not None and wsa_server.get_instance().get_client_output(interact.data.get("user")):
                # HttpValue 只发送相对路径，让前端用当前配置的 API 地址拼接，避免在后端写死 IP
                http_value = '/audio/' + os.path.basename(file_url)
                content = {
                    'Topic': 'human',
                    'Data': {
                        'Key': 'audio',
                        ...
                        'IsEnd': 1 if interact.data.get("isend", False) else 0,
                        'CONV_ID': self.user_conv_map[interact.data.get("user", "User")]["conversation_id"],
                        'CONV_MSG_NO': self.user_conv_map[interact.data.get("user", "User")]["conversation_msg_no"],
                    },
    ...
    def __process_text_output(self, text, username, uid, content_id, type):
        ...
        """
        if text:
            text = self.__normalize_childlike_tone_text(text).strip()
