...
class MyServer:
    ...
    # 接收处理（新版 websockets 不传 path，仅传 connection）
    ...
    async def __handler(self, websocket):
        """连接处理器。新版 websockets asyncio API 只传入 connection，不传 path。"""
        self.isConnect = True
    ...
    # 创建server（在事件循环内创建 websockets.serve，避免 get_running_loop 报错）
    def __connect(self):
        ...
        self.__event_loop.run_until_complete(_run_server())
