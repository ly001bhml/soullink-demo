  ...
  return (
    <div className={positionClass}>
      <div className="rounded-2xl border border-white/60 bg-white/82 p-3 shadow-xl backdrop-blur-md">
        ...
        <div className="mt-2 text-center text-sm font-bold text-gray-700">当前情绪: {emotion}</div>
        <div className="mt-1 text-center text-[11px] text-gray-500">
          最近窗口: {Math.min(frameProgress, FRAME_BATCH_SIZE)}/{FRAME_BATCH_SIZE}
        </div>
        <div className="mt-1 text-center text-sm font-bold text-teal-600">语音情绪: {voiceEmotion}</div>
        {voiceTranscript ? (
          <div className="mt-1 line-clamp-2 text-center text-[11px] text-gray-500">识别文本: {voiceTranscript}</div>
        ) : null}
