...
import { loadFusedSupportState, loadInitialMultimodalState, loadWorkshopStateSnapshot } from '../services/supportState';
...
const buildHomeEmotionPrompt = (requestText: string) => {
  ...
  const initialMultimodalState = loadInitialMultimodalState();
  const workshopSnapshot = loadWorkshopStateSnapshot();
  const fusedState = loadFusedSupportState();
  const cameraEmotionLabel = isUsableEmotionLabel(initialMultimodalState?.videoEmotionLabel)
    ? String(initialMultimodalState?.videoEmotionLabel).trim()
    : '';
  const audioEmotionLabel = isUsableEmotionLabel(initialMultimodalState?.voiceEmotionLabel)
