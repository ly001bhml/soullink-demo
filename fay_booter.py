...
def accept_audio_device_output_connect():
    ...
    try:
        ...
        util.log(1, "远程音频设备监听服务已启动，端口: 10001")
        addr = None        
        ...
        while __running:
            try:
                ...
                if __running:  # 只有在运行状态才记录错误
                    util.log(1, f"接受远程音频设备连接时出错: {e}")
                pass
    except OSError as e:
        if e.errno == 10048:  # Windows错误码：端口已被使用
            util.log(1, f"端口10001已被占用，远程音频设备监听服务未启动。错误: {e}")
            util.log(1, "提示：如果不需要远程音频设备功能，可以忽略此错误")
        else:
            util.log(1, f"启动远程音频设备监听服务失败: {e}")
    except Exception as e:
        util.log(1, f"启动远程音频设备监听服务时发生未知错误: {e}")
